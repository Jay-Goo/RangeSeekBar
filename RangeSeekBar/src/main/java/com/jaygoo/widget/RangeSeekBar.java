...
public class RangeSeekBar extends View {
  ...
  private void initProgressLine() {
    ...
    if (seekBarMode == SEEKBAR_MODE_SINGLE) {
      lineTop = (int) (leftSB.getIndicatorHeight()
          + leftSB.getIndicatorArrowSize()
          + leftSB.getThumbSize() * leftSB.getThumbScaleRatio() / 2 - progressHeight / 2);
    } else {
      lineTop = (int) (Math.max(leftSB.getIndicatorHeight()
          + leftSB.getIndicatorArrowSize()
          + leftSB.getThumbSize() * leftSB.getThumbScaleRatio() / 2, rightSB.getIndicatorHeight()
          + rightSB.getIndicatorArrowSize()
  ...
  private void initAttrs(AttributeSet attrs) {
    ...
    progressDefaultColor =
        t.getColor(R.styleable.RangeSeekBar_rsb_progress_default_color, 0xFFD7D7D7);
    ...
    tickMarkGravity =
        t.getInt(R.styleable.RangeSeekBar_rsb_tick_mark_gravity, TRICK_MARK_GRAVITY_CENTER);
      ...
      for (int i = 0; i < tickMarkTextArray.length; i++) {
        ...
        if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
          ...
          if (Utils.compareFloat(num, states[0].value) != -1
              && Utils.compareFloat(num, states[1].value) != 1
              && seekBarMode == SEEKBAR_MODE_RANGE) {
        ...
        if (isTopAndBelow != null && isTopAndBelow) {
          if (isFromFirstItem != null && isFromFirstItem && i % 2 == 0) {
            y = getLineBottom() + 1.5f * tickMarkTextMargin;
          } else if (isFromFirstItem != null && !isFromFirstItem && i % 2 != 0) {
            y = getLineBottom() + 1.5f * tickMarkTextMargin;
          } else {
  ...
  public void setTickMarkTopAndBelow(boolean isTopAndBelow, boolean isFromFirstItem) {
    this.isTopAndBelow = isTopAndBelow;
    this.isFromFirstItem = isFromFirstItem;
  }
  ...
  public void setRange(float min, float max, float interval, int tickMarkNumber) {
    ...
    if (interval >= max - min) {
      throw new IllegalArgumentException(
          "setRange() interval must be less than (max - min) ! #interval:"
              + interval
              + " #max - min:"
              + (max - min));
    }
    ...
    minRangeCells =
        (int) (reservePercent / cellsPercent + (reservePercent % cellsPercent != 0 ? 1 : 0));
  ...
  private void scaleCurrentSeekBarThumb() {
    if (currTouchSB != null && currTouchSB.getThumbScaleRatio() > 1f && !isScaleThumb) {
      isScaleThumb = true;
      currTouchSB.setThumbSize(
          (int) (currTouchSB.getThumbSize() * currTouchSB.getThumbScaleRatio()));
    ...
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        ...
        if (isOneTouchMode && seekBarMode == SEEKBAR_MODE_SINGLE) {
          ...
          touchResult = true;
          scaleCurrentSeekBarThumb();
        } else if (seekBarMode == SEEKBAR_MODE_RANGE && rightSB.collide(getEventX(event),
            getEventY(event))) {
          currTouchSB = rightSB;
          touchResult = true;
          scaleCurrentSeekBarThumb();
